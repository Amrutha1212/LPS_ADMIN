package com.javacloud.lps.rest;
...
import com.javacloud.lps.exception.LoanProgNotFoundException;
...
public class AdminController {
	...
	public Response<User> forgotPassword(@PathVariable String email, @PathVariable String password,
			@RequestBody User theUser) {

		theUser = userService.findByEmail(email);

		if (theUser != null) {
			userService.forgotPassword(theUser);
			theUser.setPassword(password);
			return new Response<>(false, "successfully saved", theUser);

		} else {
			return new Response<>(true, "Email does not exits", null);
		}
	}
